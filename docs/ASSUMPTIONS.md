...
| 17 | **pnpm build-script approval** is pre-granted only for `@discordjs/opus`, `argon2` and `esbuild` (`onlyBuiltDependencies` in pnpm-workspace.yaml). | pnpm 10 blocks dependency install scripts by default (supply-chain guard); these three legitimately need them (native compile / binary setup). | Add new entries to the list when a future dependency needs scripts. |
| 18 | **App bundles contain only workspace code** (`tsup skipNodeModulesBundle`); all npm packages are runtime `node_modules` in the images. | Inlining CommonJS deps (pg) into ESM bundles breaks at runtime ("Dynamic require … not supported"); externals also keep native modules loadable. | Don't change without re-validating the prod images. |
| 19 | **Login rate limit is 20/minute/IP** (not lower). | Leaves headroom for the e2e suite's repeated logins while still stopping brute force; failures are additionally audited. | One constant in `apps/admin/src/server.ts`. |
